...
static float * audio_mono_from_planar(float * raw, int T, int sr, int target_sr, int * T_out) {
    ...
    int     T_rs      = T;
    if (sr != target_sr) {
        qt_log(QT_LOG_INFO, "[Resample] %d Hz -> %d Hz, %d samples...", sr, target_sr, T);
        int     T_new     = 0;
        float * resampled = audio_resample(raw, T, sr, target_sr, 2, &T_new);
        free(raw);
        if (!resampled) {
            qt_log(QT_LOG_ERROR, "[Resample] Resample failed");
            *T_out = 0;
            return NULL;
        }
        qt_log(QT_LOG_INFO, "[Resample] Done: %d -> %d samples", T, T_new);
        stereo_rs = resampled;
        T_rs      = T_new;
    }
...
static bool wav_stream_write_header(wav_stream * ws) {
    ...
    if (fwrite(header, 1, 44, ws->fp) != 44) {
        qt_log(QT_LOG_ERROR, "[WAVStream] header write failed");
        return false;
    }
    fflush(ws->fp);
    return true;
...
static bool wav_stream_open_stdout(wav_stream * ws, int sr, WavFormat fmt) {
    ...
    const char * fmt_name = (fmt == WAV_S16) ? "S16" : (fmt == WAV_S24) ? "S24" : "F32";
    qt_log(QT_LOG_INFO, "[WAVStream] stdout: %d Hz, mono %s", sr, fmt_name);
    return true;
}
